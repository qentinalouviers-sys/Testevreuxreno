...
export function ProducerCard({ producer }: { producer: Producer }) {
  ...
  return (
    <Link
      ...
      className="group flex h-full flex-col border border-ink/10 bg-white p-7
                 transition-all duration-600 hover:border-gold-600/50 hover:bg-paper-2 sm:p-8"
    >
      <span className="flex items-start justify-between gap-4">
        <Rosette className="size-9 text-gold-500 transition-transform duration-[1.4s] group-hover:rotate-45" />
        <Flag code={producer.flag} className="mt-1 h-3.5 w-[21px]" />
      </span>

      <h3 className="mt-6 font-display text-2xl text-ink">{producer.name}</h3>
      <p className="mt-1.5 text-[0.6rem] uppercase tracking-[0.2em] text-gold-600">
      ...
      <p className="mt-5 text-sm leading-relaxed text-ink-mute">{copy?.bio}</p>

      <div className="mt-auto flex items-end justify-between gap-4 pt-7">
        <span className="text-[0.62rem] uppercase tracking-[0.16em] text-ink-mute">
      ...
      {producer.payouts === "active" && (
        <span className="mt-4 inline-flex items-center gap-2 text-[0.58rem] uppercase tracking-[0.14em] text-olive-600">
